...
double SmoothingSpline::g(double m0, double m1, double f0, double f1, double u, double x2, double x3, double w)
{
    return (m0*((x3-w)*(x3-w)*(x3-w)))/(6*u)+m1*((w-x2)*(w-x2)*(w-x2))/(6*u)+(f0-m0*u*u/6)*(x3-w)/u+(f1-m1*u*u/6)*(w-x2)/u;
}

void SmoothingSpline::setSP2D(QFile InputFile, double error)
{
    ...
    while (!inXY.atEnd()) {
        ...
        x.append(strX.toDouble());
        y.append(strY.toDouble());
        //z.append(strZ.toDouble());
        i++;
    }
    //qDebug() << x;
    //qDebug() << y;
...
void SmoothingSpline::buildSP2D()
{
    ...
    for (i = 0; i <=l-1; i++)
    {
        for (j = 0; j <= l-1; j++)
        {
            ...
            if (i==j) gsl_matrix_set (P, i, j, 1/p);
...
/* void SmoothingSpline::setSP3D(QFile InputFile, double error)
{
